...
ok = True
counts = list()
def thread_function(index, soundFile, event):
    try:
        counts[index] = counts[index] + transformSoundFile(soundFile)
    except KeyboardInterrupt as exitExc:
        ok = False
    event.set()

def except_function(soundFiles):
    cnt = 0
    for soundFile in soundFiles:
        outFile = soundFile.replace('in' + sep, 'out' + sep, 1)
        if os.path.exists(outFile):
            os.remove(outFile)
        cnt = cnt + transformSoundFile(soundFile)
    return cnt
    
# transform files
threadCount = 2
count = 0
length = len(listOfSoundFiles)
for index in range(0, length - threadCount, threadCount):
    soundFiles = list()
    counts.clear()
    for i in range(threadCount):
        soundFiles.append(listOfSoundFiles[index + i])
        counts.append(0)
    if count < 19000 or count >= 22000:
    ...
    try:
        ...
        for i in range(threadCount):
            ...
            x = threading.Thread(target=thread_function, args=(i,soundFile,event,))
            threads.append(x)
            x.start()
